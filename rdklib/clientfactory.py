...
class ClientFactory:
    ...
    def build_client(self, service, region=None):
...
def get_assume_role_credentials(role_arn, region=None):
    try:
        sts_client = boto3.client('sts', region_name=region, endpoint_url=f"https://sts.{region}.amazonaws.com" if region else None)
